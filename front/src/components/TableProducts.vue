                ...
                <v-container>
                    ...
                    <v-form ref='form'>
                        <v-row dense>
                            ...
                            <v-col cols="12">
                            <v-text-field v-model="editedItem.price" label="Preço*" required></v-text-field>
                            </v-col>
                        </v-row>  
                    </v-form>
                </v-container>
...
export default {
    ...
    methods: {
        fetchProdutos() {
            // Fazer a requisição para a API /produtos e atribuir os dados à propriedade Produtos
        ...
        async save() {
            ...
            axios.post('/produtos', {
                ...
                .then(response => { 
                  this.message = `Produto ${response.data.name} cadastrado com sucesso!`;
                  this.fetchProdutos();
        ...
        async update() {
            this.editedItem.price = parseFloat(this.editedItem.price);
            axios.put(`/produtos/${this.editedItem.id}`, {
                ...this.editedItem
            })
                .then(response => {
                    this.message = `Produto ${response.data.name} atualizado com sucesso!`;
                    this.fetchProdutos();
                    this.typemessage = 'success';
                    this.dialog = false;
                    this.$refs.form.reset();
        ...
        deleteItemConfirm () {
            ...
            axios.delete(`/produtos/${this.editedItem.id}`)
                .then(response => {
                    //faz uma copia do nome do usuario para exibir na mensagem
                    
                    this.message = `Produto ${name} excluído com sucesso!`;
                    this.typemessage = 'success';
    ...
    watch: {
      ...
      Produtos (val) {
        //orderna a lista de Produtos
        this.Produtos.sort((a, b) => b.id > a.id);
      },
